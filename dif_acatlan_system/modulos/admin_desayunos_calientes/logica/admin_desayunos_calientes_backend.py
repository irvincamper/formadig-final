...
# Lógica de Backend de Python - Desayunos Calientes
app = Flask(__name__)
...
def obtener_registros():
    ...
    try:
        ...
        res = supabase.table('desayunos_sieb').select('*').eq('tipo_apoyo', 'Calientes').order('fecha_registro', desc=True).execute()
        return jsonify({"desayunos": res.data}), 200
    except Exception as e:
        print(f"❌ Error GET Calientes: {e}")
        return jsonify({"error": str(e)}), 500

@app.route('/api/desayunos_calientes', methods=['POST'])
def nuevo_registro():
    if not supabase:
        return jsonify({"error": "Supabase no configurado"}), 500
    ...
    try:
        print(f"DEBUG: Intentando insertar en desayunos_sieb (Calientes): {data.get('nombre_beneficiario')}")
        ...
        print(f"DEBUG: Enviando a Supabase: {json.dumps(supabase_data, indent=2)}")
        ...
        print(f"❌ ERROR Supabase Calientes: {e}")
        return jsonify({"error": str(e)}), 500
...
def dictamen_registro(record_id):
    ...
    try:
        update_data = {
            "localidad": data.get('comisaria') # Se mapea a localidad en el esquema real
        }
...
if __name__ == '__main__':
    print("🔥 Servidor Python iniciado en el puerto 5006 - Desayunos Calientes (SUPABASE READY)")

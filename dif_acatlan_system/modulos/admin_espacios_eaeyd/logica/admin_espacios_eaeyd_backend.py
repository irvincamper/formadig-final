...
# Lógica de Backend de Python - Espacios EAEyD
app = Flask(__name__)
...
def obtener_registros():
    ...
    try:
        ...
        res = supabase.table('desayunos_sieb').select('*').eq('tipo_apoyo', 'EAEyD').order('fecha_registro', desc=True).execute()
        return jsonify({"desayunos": res.data}), 200
    except Exception as e:
        print(f"❌ Error GET EAEyD: {e}")
        return jsonify({"error": str(e)}), 500

@app.route('/api/espacios_eaeyd', methods=['POST'])
def nuevo_registro():
    if not supabase:
        return jsonify({"error": "Supabase no configurado"}), 500
    ...
    try:
        print(f"DEBUG: Intentando insertar en desayunos_sieb (EAEyD): {data.get('nombre_beneficiario')}")
        ...
        supabase_data = {
            ...
            "curp": data.get('curp'),
            "localidad": data.get('escuela'), # Se usa escuela para la sede en el frontend
            "tipo_apoyo": "EAEyD",
        ...
        print(f"❌ ERROR Supabase EAEyD: {e}")
        return jsonify({"error": str(e)}), 500
...
if __name__ == '__main__':
    print("🔥 Servidor Python iniciado en el puerto 5007 - Espacios EAEyD (SUPABASE READY)")

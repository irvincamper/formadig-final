...
document.addEventListener('DOMContentLoaded', () => {
    const nutriForm = document.getElementById('nutritionForm') as HTMLFormElement;
    const beneficiarySelect = document.getElementById('nutriBeneficiary') as HTMLSelectElement;
    ...
    const alertsContainer = document.getElementById('nutritionAlerts') as HTMLDivElement;
    ...
    function populateBeneficiaries() {
        ...
        if (typeof (window as any).beneficiariesDb !== 'undefined') {
            const actives = (window as any).beneficiariesDb.filter((b: any) => b.status === 'Activo');
            actives.forEach((b: any) => {
    ...
    function calcularIMC(nombre: string, peso: number, talla: number) {
    ...
    function generarAlertaUICard(nombre: string, peso: number, talla: number, imc: string, clasificacion: string, tipo: string) {
    ...
    function showError(inputElement: HTMLElement, errorId: string) {
        inputElement.classList.add('form__input--invalid');
        const errorEl = document.getElementById(errorId);
        if (errorEl) errorEl.classList.add('form__error--active');
    }

    function hideError(inputElement: HTMLElement, errorId: string) {

...
@desayunos_frios_bp.route('/', methods=['GET'], strict_slashes=False)
def obtener_registros():
    try:
        # Query tabla principal desayunos_frios con límite de 1000 registros
        res = global_client.table('desayunos_frios').select('*').order('fecha_registro', desc=True).limit(1000).execute()
...
@desayunos_frios_bp.route('/<string:record_id>', methods=['GET'], strict_slashes=False)
def obtener_desayuno(record_id):
    """Obtener UN desayuno frío específico por ID"""
    try:
        res = global_client.table('desayunos_frios').select('*').eq('id', record_id).limit(1).execute()
        
        if not res.data:
            return jsonify({"error": f"Desayuno con ID {record_id} no encontrado"}), 404
        
        r = res.data[0]
        print(f"📍 GET /api/desayunos_frios/{record_id} → Desayuno frío cargado")
...
@desayunos_frios_bp.route('/<string:record_id>', methods=['PUT', 'PATCH'], strict_slashes=False)

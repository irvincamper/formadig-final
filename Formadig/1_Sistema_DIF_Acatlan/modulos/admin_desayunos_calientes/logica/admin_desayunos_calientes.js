...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTabla(registros) {
        ...
        // Guardar todos para paginación (pero permitiendo filtrados)
        ...
        datosPagina.forEach(r => {
            ...
            tr.addEventListener('click', () => {
                ...
                renderDocBtn('btnDocSaludCont',       r.url_salud || r.url_comprobante_salud,  'Comprobante Salud');
                renderDocBtn('btnDocIneTutorCont',    r.url_ine   || r.foto_ine_url || r.url_ine_tutor, 'INE del Tutor');
    ...
    async function autollenarPorFolio(session) {
        ...
        if (folioId) {
            console.log("🔍 DESAYUNOS CALIENTES - Detectado ID en URL:", folioId);
            try {
                const response = await fetch(`/api/desayunos_calientes/${folioId}`, {
                    headers: { 'Authorization': `Bearer ${session.token}` }
                });

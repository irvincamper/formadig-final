...
function MainLayout() {
  ...
  const [activeView, setActiveView] = useState<'admin' | 'nutrition' | 'appointments' | 'reports' | 'breakfasts' | 'breakfasts_calientes' | 'traslados' | 'eaeyd'>('admin');
    ...
    <div className="app-container">
      ...
      <aside className="sidebar">
        ...
        <nav className="nav-menu">
          ...
          <div className="sidebar-section-label">Programas Alimentarios</div>
          {canViewBreakfasts && (
            <div 
            ...
            >
              <span className="nav-item-icon">🥛</span>
              <span>Desayunos Fríos</span>
            </div>
          )}

          {canViewBreakfasts && (
            <div 
              className={`nav-item ${activeView === 'breakfasts_calientes' ? 'active' : ''}`}
              onClick={() => setActiveView('breakfasts')} // Simplified: both go to same dashboard for now
            >
              <span className="nav-item-icon">🍲</span>
              <span>Desayunos Calientes</span>
            </div>
          )}
          ...
          <div className="sidebar-section-label">Desarrollo Integral</div>
          {canViewNutrition && (
            <div 
          ...
          <div 
            className={`nav-item ${activeView === 'eaeyd' ? 'active' : ''}`}
            onClick={() => setActiveView('admin')} // Placeholder for EAeyD
          >
            <span className="nav-item-icon">🏢</span>
            <span>EAEyD</span>
          </div>
